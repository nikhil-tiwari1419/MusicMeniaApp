    ...
    return (
        <div className={`border-b ${dark ? "border-gray-700" : "border-gray-200"}`}>
            ...
            <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96 pb-4" : "max-h-0"}`}>
                <p className={`px-2 text-xl sm:text-2xl leading-relaxed ${dark ? "text-white" : "text-black"}`}>{answer}</p>
            </div>
        </div>
    )
}
        ...
        <div className={` py-10 px-5 ${dark ? "bg-gray-800 text-white" : "bg-white text-black"}`}>
            <div className='max-w-2xl mx-auto'>
                ...
                <div className='mb-8 text-center'>
                    ...
                    <p className='text-xl font-semibold'>Can't find the answer? Contact our support Team. </p>
                </div>
                ...
                <div className='flex flex-wrap gap-12 mb-8 justify-center'>
                    {categories.map(cat => (
                        <button
                            ...
                            className={`${activeCategory === cat ? "border-blue-700 border-b-2 cursor-pointer " : ""} text-xl sm:text-2xl font-semibold capitalize px-2 py-1.5 `}
                        >
                            {cat}
                        </button>
                    ))}
                ...
                >
                    {filteredQna.length > 0 ? filteredQna.map(q => (
                        <FAQItem
                            ...
                            className={`text-2xl md:text-4xl ${dark ? "text-white":"text-black"}`}
                        />
                    )) : (
