...
function DesktopMusicCard({ music, isPlaying, isActuallyPlaying, onPlay, dark, progress, isLiked, onToggleLike }) {
    return (
        <div
            className={`group relative border-2 border-black transition-all duration-150 cursor-pointer
                ${isPlaying
                    ? 'bg-yellow-400 shadow-[4px_4px_0_#000]'
                    : dark
        ...
        >
            {/* Thumbnail */}
            <div className="relative aspect-square overflow-hidden border-b-2 border-black bg-zinc-800">
                {music.thumbnail
                    ? <img src={music.thumbnail} alt={music.title} loading="lazy"
                ...
                <div onClick={() => onPlay(music)}
                    ...
                    <button
                        className={`w-12 h-12 border-2 border-black flex items-center justify-center transition-all duration-150
                            bg-yellow-400 shadow-[3px_3px_0_#000]
...
export default function DesktopMusicLayout({
    ...
    const btnBase = `border-2 border-black font-black text-xs uppercase tracking-widest transition-all duration-100
